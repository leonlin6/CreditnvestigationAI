...
import { establishReport } from "../services/langchainService.js";
import { excuteNewDoc } from "../services/documentService.js";

export const getReport = async (db: SqlDatabase, year: number, companyName: string) => {
  ...
  const report = await establishReport(Number(year), gui_no, db).catch((err) => {
    console.log(err);
    return err.message;
  });
